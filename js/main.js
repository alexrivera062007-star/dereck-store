...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // Product card: first tap reveals the "Comprar" icon, second tap (on the icon) opens the modal
  document.querySelectorAll('.product-card').forEach(card => {
    card.style.cursor = 'pointer';
    const imgWrap = card.querySelector('.product-image-wrap');

    card.addEventListener('click', (e) => {
      // Don't trigger if clicking the wishlist heart
      if (e.target.closest('.heart-btn') || e.target.closest('.wishlist-heart')) return;

      // Click was directly on the "Comprar" icon/button -> open the modal
      ...
      // Click was on the photo/card body: reveal the icon first, don't open the modal yet
      if (imgWrap && !imgWrap.classList.contains('overlay-active')) {
        e.preventDefault();
        // Hide the icon on any other card that had it open
