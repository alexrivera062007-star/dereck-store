...
// --- Recargo por talla ---
// Si una talla debe costar más (ej. XL usa más tela), agrégala aquí.
// Esto aplica a TODOS los productos que tengan esa talla, a menos que
// el producto tenga su propio recargo con el atributo data-size-extra
// (ver README, sección "Precio distinto según talla").
const SIZE_SURCHARGES = {
  "XL": 10,
  "XXL": 15
};

// Devuelve el recargo (en soles) que corresponde a una talla para un producto dado
function getSizeExtra(product, size) {
  if (!size) return 0;
  if (product.sizeExtra && Object.prototype.hasOwnProperty.call(product.sizeExtra, size)) {
    return Number(product.sizeExtra[size]) || 0;
  }
  return SIZE_SURCHARGES[size] || 0;
}

// Precio final de una unidad, según la talla seleccionada
function currentUnitPrice() {
  if (!currentProduct) return 0;
  return currentProduct.price + getSizeExtra(currentProduct, currentProduct.selectedSize);
}
...
// Loads products from each category HTML file so index.html
// "Todos" is always in sync — just edit jeans.html, buzos.html, etc.
// ============================================================
...
async function loadAllProducts() {
  ...
  // Only run on index (grid starts with the loading spinner)
  if (!grid.querySelector('.grid-loading')) return;
  ...
  for (const file of CATEGORY_FILES) {
    try {
      ...
      cards.forEach(card => allCards.push(card.outerHTML));
    } catch (e) {
...
function initProductCards() {
  document.querySelectorAll('.product-card').forEach(card => {
    ...
    card.addEventListener('click', (e) => {
      if (e.target.closest('.heart-btn') || e.target.closest('.wishlist-heart')) return;

      if (e.target.closest('.quick-add')) {
        e.preventDefault();
        openProductModal(card.querySelector('.quick-add'));
        return;
      }

      if (imgWrap && !imgWrap.classList.contains('overlay-active')) {
        e.preventDefault();
        document.querySelectorAll('.product-image-wrap.overlay-active').forEach(el => {
          if (el !== imgWrap) el.classList.remove('overlay-active');
        });
        imgWrap.classList.add('overlay-active');
      }
    });
  });
}
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // Load products dynamically from category files (only on index.html)
  loadAllProducts();

  // Tapping anywhere outside a product card hides any open "Comprar" icon
  document.addEventListener('click', (e) => {
    if (!e.target.closest('.product-card')) {
      document.querySelectorAll('.product-image-wrap.overlay-active').forEach(el => el.classList.remove('overlay-active'));
    }
  });
...
// ============================================================
// CATEGORY FILTER — no page reload
// ============================================================
...
function verOfertas(e) {
  e.preventDefault();
  const btn = document.querySelector('.pill[data-filter="oferta"]');
  filterCategory('oferta', btn);
  // Only scroll when explicitly clicking "Ver Ofertas" from the banner
...
function updateModalSummary() {
  ...
  document.getElementById('sumTotal').textContent = `S/ ${(currentUnitPrice() * modalQty).toFixed(2)}`;
...
function openProductModal(btn) {
  ...
  let sizeExtra = {};
  if (card.dataset.sizeExtra) {
    try { sizeExtra = JSON.parse(card.dataset.sizeExtra); }
    catch (e) { console.warn('data-size-extra inválido en', card.dataset.name, e); }
  }
  ...
  if (currentProduct.imgSrc) {
    ...
    modalImgWrap.innerHTML = `<img src="${currentProduct.imgSrc}" alt="${currentProduct.name}"/>`;
  } else {
    modalImgWrap.classList.add('placeholder-img');
    modalImgWrap.innerHTML = `<i class="fas fa-image"></i><span>Sin imagen</span>`;
  }

  // Sizes (muestra el recargo junto a la talla, si aplica: ej. "XL +S/10")
  const sizesWrap = document.getElementById('modalSizes');
  sizesWrap.innerHTML = currentProduct.sizes.map(s => {
    const extra = getSizeExtra(currentProduct, s);
    const label = extra > 0 ? `${s} <small>+S/${extra}</small>` : s;
    return `<button class="option-btn" onclick="selectOption('size','${s}',this)">${label}</button>`;
  }).join('');
...
function buyNowWhatsApp() {
  if (!validateSelection()) return;
  const total = currentUnitPrice() * modalQty;
...
function renderCartItems() {
  ...
  container.innerHTML = cart.map((item, i) => `
    <div class="cart-item">
      <div class="cart-item-img" onclick="viewCartProduct('${item.name}')" title="Ver producto" style="cursor:pointer;">
        ${item.img
          ? `<img src="${item.img}" alt="${item.name}" style="width:100%;height:100%;object-fit:cover;border-radius:4px"/>`
          : `<i class="fas fa-tshirt" style="font-size:1.4rem"></i>`}
        <div class="cart-item-img-overlay"><i class="fas fa-eye"></i></div>
      </div>
      <div class="cart-item-info">
        <p class="cart-item-name">${item.name}</p>
        <p class="cart-item-meta">Talla: ${item.size} · Color: ${item.color}</p>
        <p class="cart-item-price">S/ ${item.price} × ${item.qty}</p>
        <button class="cart-item-view" onclick="viewCartProduct('${item.name}')">
...
function viewCartProduct(name) {
  // Find the product card in the grid and open its modal
  const card = Array.from(document.querySelectorAll('.product-card'))
